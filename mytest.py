...
def sign_in():
    ...
    with open("sign_in.jpg", "wb") as f:
        f.write(base64.b64decode(data))
    # 这里face-recognition也是机器学习完成的， 返回数据是ndarray
    mydata = load_image_file("sign_in.jpg")
    ...
    myimg.save("sign_in_id.jpg")  # 保存切分出人脸之后得照片
    img = cv.imread("sign_in_id.jpg")
    # 保存写有类别名称的人脸图片
...
def sign_out():
    ...
    data = request.form.get("face")
    # 把数据保存成图片，前端返回的64编码数据，后端base64解码
    with open("sign_out.jpg", "wb") as f:
        f.write(base64.b64decode(data))

    # 因为数据库存储的是base64编码，不需要转成ndarray，也不需要存图片,直接连接数据库
    mydata = load_image_file("sign_out.jpg")
    # 用yolov5替换face_locations人脸识别函数
    # 返回人脸坐标和识别类别
    img = "sign_out.jpg"
    myresult = main(img)
    ...
    # ndarray转图片需要pillow
    myimg = Image.fromarray(cut_face)
    myimg.save("sign_out_id.jpg")  # 保存切分出人脸之后得照片
    img = cv.imread("sign_out_id.jpg")
    cv.putText(img=img, text=names, org=(5, 50), fontFace=cv.FONT_HERSHEY_SIMPLEX, fontScale=1, color=[0, 0, 255],
               thickness=None, lineType=cv.LINE_AA)
    cv.imwrite("sign_out_id_label.jpg", img)

    # 把图片数据回传给前端，前端接收的是base64数据，现在cut_face是ndarray类型
    # 通过分析可以知道 b.jpg编译成base64编码，再往前端发送.
    with open("sign_out_id_label.jpg", "rb") as f:
